...
async function scrapeTikTok() {
  ...
  const page = await browser.newPage();
    ...
    // Add this before navigating to TikTok
    await page.setJavaScriptEnabled(true);
    await page.setBypassCSP(true);

    // Pre-emptively set cookie preferences
    await page.evaluateOnNewDocument(() => {
    localStorage.setItem('tt_cookie_preferences', JSON.stringify({
        necessary: true,
        analytics: false,
        advertising: false,
        performance: false
    ...
    // Block cookie consent dialog from appearing
    Object.defineProperty(window, 'tiktokConsent', {
        get: () => ({
        isConsentBannerShowing: false,
        hasConsented: true
        }),
        configurable: true
    });
